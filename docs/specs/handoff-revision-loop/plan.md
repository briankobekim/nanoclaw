...
Implementation-review corrections (2026-09-15, same day, no change of intent): (a) every transition (`transition()`, `reviewHandoff`) first re-derives the row's fingerprint and refuses a row whose stored fingerprint no longer matches its fields, so a link or contract field edited after insert can never be reviewed, acknowledged, or closed (A11); (b) a successor is honored by the stall sweep and the mission view only when its own fingerprint matches its fields, via `trustedSupersededIds()`, so a corrupted link cannot hide a prior (D4, E11).
...
Failure boundaries: the hook is wrapped in the sweep's existing try/catch-and-log so a ledger or Slack failure never stops session maintenance. Implementation-review corrections (2026-09-15): each `deliver` call is bounded by `DELIVER_TIMEOUT_MS` (30 s); a call that has not settled by then is treated as a failed send (warned, unrecorded, retried next tick), so a hung Slack call cannot wedge the host sweep loop (E12). A malformed `owner_pinged` payload is warned about per event, never counts as a dedupe hit, and never stops the sweep (E13). Concurrent agent writes can race the `MAX+1` sequence (`UNIQUE(handoff_id, sequence)`) → the append throws → logged → one duplicate next tick. No in-memory state; restart-safe because dedupe lives in `handoff_events`.
...
- **A11** `a tampered supersedes link is refused by every later transition` (implementation-review correction) — after delivery, `UPDATE supersedes = NULL` makes `reviewHandoff` throw `/ledger fingerprint does not match its own fields/` and leaves status `delivered`; restored and approved, the same tamper makes `acknowledgeHandoff` throw; acknowledged, a tampered `goal` makes `closeHandoff` throw and status stays `acknowledged`.
...
- **D4** `a corrupted successor link does not hide the prior` (implementation-review correction) — an unrelated row pointed at a `changes_required` prior by direct SQL (its fingerprint no longer matches) leaves the prior at stage `changes_requested` with the `--supersedes` next action.
...
- **E11** `a corrupted successor link does not silence the prior` (implementation-review correction) — an unrelated row pointed at a stalled handoff by direct SQL: the stalled handoff is still pinged once and a warning names the corrupted successor.
- **E12** `a delivery that never settles is treated as a failure and retried` (implementation-review correction) — a `deliver` promise that never resolves, with the timeout injected at 20 ms: the sweep returns, warns with the handoff id, records no event; the next sweep sends once and records one event.
- **E13** `a malformed owner_pinged event neither blocks the sweep nor counts as a ping` (implementation-review correction) — a hand-inserted `owner_pinged` row with invalid JSON on handoff A: both stalled handoffs A and B are pinged, one valid event is recorded for A, a warning names the bad event, and a second sweep sends nothing.
