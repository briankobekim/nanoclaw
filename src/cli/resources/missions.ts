...
function handoffStage(row: HandoffRow): string {
  if (row.reservation_kind === 'review') return 'review_in_transit';
  switch (row.status) {
    case 'created':
...
function handoffNextAction(row: HandoffRow): string {
  if (row.reservation_kind === 'delivery') return 'await delivery receipt or retry after lease';
  if (row.reservation_kind === 'review') return 'await review receipt or retry after lease';
  switch (row.status) {
    case 'created':
...
function handoffState(row: HandoffRow): string {
  if (row.reservation_kind === 'delivery') return 'delivering';
  if (row.reservation_kind === 'review') return `reviewing: ${row.review_outcome ?? 'unknown'}`;
  return row.review_outcome ? `${row.status}: ${row.review_outcome}` : row.status;
}
...
function handoffMission(row: HandoffRow, names: Map<string, string>): MissionListRow {
  return {
    ...
    class: 'unknown',
    project: row.project,
    stage: handoffStage(row),
    handoff_state: handoffState(row),
    next_action: handoffNextAction(row),
