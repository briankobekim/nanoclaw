...
import { reserveHandoffReview } from '../../modules/handoff-ledger/ledger.js';
...
const SOURCE = 'ag-source-missions';
const REVIEWER = 'ag-reviewer-missions';
...
beforeEach(async () => {
  ...
  await createGroup(SOURCE, 'Source');
  await createGroup(REVIEWER, 'Reviewer');
...
describe('missions CLI', () => {
  ...
  it('derives the review stages while keeping actual shipping authority unknown after approval', async () => {
    ...
    const response = await dispatch({ id: 'missions-approved', command: 'missions-list', args: {} }, agent(SOURCE));
    expect(response.ok).toBe(true);
    if (!response.ok) return;
  ...
  it('shows an active review reservation as in transit without claiming review completion', async () => {
    const { fingerprint } = await createHandoff('MISSION-REVIEW-IN-TRANSIT');
    await dispatch(
      {
        id: 'deliver-review-in-transit',
        command: 'handoffs-deliver',
        args: { id: 'MISSION-REVIEW-IN-TRANSIT', fingerprint },
      },
      agent(SOURCE),
    );
    await reserveHandoffReview('MISSION-REVIEW-IN-TRANSIT', REVIEWER, fingerprint, 'APPROVED');

    const response = await dispatch(
      { id: 'missions-review-in-transit', command: 'missions-list', args: {} },
      agent(SOURCE),
    );
    expect(response.ok).toBe(true);
    if (!response.ok) return;
    expect(response.data).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          mission_id: 'MISSION-REVIEW-IN-TRANSIT',
          stage: 'review_in_transit',
          handoff_state: 'reviewing: APPROVED',
          shipping_allowed: 'no',
          next_action: 'await review receipt or retry after lease',
        }),
      ]),
    );
  });
  ...
  it('derives changes-requested, blocked, and completed stages from exact ledger states', async () => {
    ...
    const response = await dispatch({ id: 'missions-stages', command: 'missions-list', args: {} }, agent(SOURCE));
    expect(response.ok).toBe(true);
    if (!response.ok) return;
  ...
  it('scopes agent callers to involved handoffs and their own tasks', async () => {
    ...
    const reviewerView = await dispatch(
      { id: 'missions-reviewer-filter', command: 'missions-list', args: { group: 'Reviewer' } },
      { caller: 'host' },
    );
    expect(reviewerView.ok).toBe(true);
    if (reviewerView.ok) {
      expect(reviewerView.data).toEqual(
        expect.arrayContaining([expect.objectContaining({ mission_id: 'MISSION-PRIVATE', owner: 'Source' })]),
      );
    }
  });
  ...
  it('ages terminal change-requested and review-blocked handoffs out of the recent window', async () => {
    for (const outcome of ['CHANGES REQUIRED', 'REVIEW BLOCKED'] as const) {
      ...
      await dispatch({ id: `deliver-${id}`, command: 'handoffs-deliver', args: { id, fingerprint } }, agent(SOURCE));
      await dispatch(
  ...
  it('keeps orphaned durable handoffs in the unfiltered host view with owner ID fallback', async () => {
    ...
    await deleteAgentGroup(SOURCE);
    await deleteAgentGroup(REVIEWER);
    ...
    expect(response.data).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ mission_id: 'MISSION-ORPHANED', owner: SOURCE, owner_agent_group_id: SOURCE }),
      ]),
    );
  });
  ...
  it('uses the richer handoff row when its source session exactly matches a task mission', async () => {
    ...
    const response = await dispatch({ id: 'missions-linked', command: 'missions-list', args: {} }, agent(SOURCE));
    expect(response.ok).toBe(true);
    if (!response.ok) return;
  ...
  it('does not hide unrelated series when a handoff points at a legacy shared task session', async () => {
    ...
    await withMailboxSession(SOURCE, sessionId, async (mailbox) => {
    ...
    const response = await dispatch({ id: 'missions-shared', command: 'missions-list', args: {} }, agent(SOURCE));
    expect(response.ok).toBe(true);
    if (!response.ok) return;
