...
import type { ChannelSetup, InboundDeliveryReceipt, InboundMessage } from './adapter.js';
...
export type SlackBotInboundDecision =
  ...
  | {
       ...
       */
      senderId?: string;
      /** Agent group that must appear in the host's durable delivery receipt. */
      requiredAgentGroupId?: string;
      /**
       * Optional fail-closed checkpoint run before the message reaches the
       * router. A rejection or throw stops delivery. Feature modules use this
       * for host-owned state transitions that must be committed before an
       * agent can act on the message (for example, a trusted handoff ledger).
       */
      ...
      /**
       * Called when a policy admitted a message but its pre-forward checkpoint
       * or downstream delivery failed. Policies that reserve scarce capacity
       * during admission use this to release it.
       */
...
export function wrapSlackBotGuard(setup: ChannelSetup, instanceKey: string): ChannelSetup {
  return {
    ...
    async onInbound(platformId: string, threadId: string | null, message: InboundMessage) {
      ...
      try {
        decision = await botInboundPolicy.decideBotInbound({
          instanceKey,
          platformId,
          threadId,
          message,
      ...
      if (decision.beforeForward) {
        try {
          await decision.beforeForward();
        } catch (err) {
          await decision.onRejected?.();
          log.warn('slack-a2a-guard: pre-forward checkpoint failed — dropping bot-authored inbound', {
            instanceKey,
            platformId,
            botId: bot.botId,
            err,
      ...
      let result: Awaited<ReturnType<ChannelSetup['onInbound']>>;
      try {
        result = await setup.onInbound(platformId, threadId, message);
      } catch (err) {
        await decision.onRejected?.();
        throw err;
      }
      if (decision.requiredAgentGroupId && !isDeliveryReceiptFor(result, decision.requiredAgentGroupId)) {
        await decision.onRejected?.();
        log.warn('slack-a2a-guard: required recipient did not accept bot-authored inbound', {
          instanceKey,
          platformId,
          botId: bot.botId,
          requiredAgentGroupId: decision.requiredAgentGroupId,
        });
        return;
      }
...
function isDeliveryReceiptFor(value: unknown, agentGroupId: string): value is InboundDeliveryReceipt {
  if (!value || typeof value !== 'object') return false;
  const ids = (value as Partial<InboundDeliveryReceipt>).deliveredAgentGroupIds;
  return Array.isArray(ids) && ids.includes(agentGroupId);
}
