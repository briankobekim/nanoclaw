...
describe('wrapSlackBotGuard — admission-policy seam', () => {
  ...
  it('runs an asynchronous pre-forward checkpoint before downstream delivery', async () => {
    const order: string[] = [];
    const { setup } = makeSetup(async () => {
      order.push('downstream');
    });
    ...
    expect(order).toEqual(['checkpoint', 'downstream']);
  });

  it('fails closed when a pre-forward checkpoint rejects', async () => {
    const { setup, calls } = makeSetup();
    const wrapped = wrapSlackBotGuard(setup, 'slack');
    setBotInboundPolicy({
      decideBotInbound: () => ({
        action: 'admit',
        beforeForward: async () => {
          throw new Error('ledger rejected package');
        },
      }),
    });

    await expect(wrapped.onInbound('slack:G7', null, botMessage())).resolves.toBeUndefined();

    expect(calls).toHaveLength(0);
    expect(log.warn).toHaveBeenCalledWith(expect.stringContaining('pre-forward checkpoint failed'), expect.anything());
  });
  ...
  it('does not call onAccepted when downstream throws — the error propagates', async () => {
    ...
    const wrapped = wrapSlackBotGuard(setup, 'slack');
    setBotInboundPolicy({
      decideBotInbound: () => ({ action: 'admit', onAccepted, onRejected }),
    });
  ...
  it('requires a durable receipt for the policy-selected recipient', async () => {
    const onAccepted = vi.fn();
    const onRejected = vi.fn();
    const { setup } = makeSetup(async () => ({ deliveredAgentGroupIds: ['ag-someone-else'] }));
    const wrapped = wrapSlackBotGuard(setup, 'slack');
    setBotInboundPolicy({
      decideBotInbound: () => ({
        action: 'admit',
        requiredAgentGroupId: 'ag-reviewer',
        onAccepted,
        onRejected,
      }),
    });
  ...
  it('accepts a durable receipt naming the policy-selected recipient', async () => {
    const onAccepted = vi.fn();
    const onRejected = vi.fn();
    const { setup } = makeSetup(async () => ({ deliveredAgentGroupIds: ['ag-reviewer'] }));
    const wrapped = wrapSlackBotGuard(setup, 'slack');
    setBotInboundPolicy({
      decideBotInbound: () => ({
        action: 'admit',
