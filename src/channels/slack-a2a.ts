...
export type A2aMessageEnforcementDecision =
  | { action: 'allow'; beforeForward?: () => void | Promise<void>; onAccepted?: () => void | Promise<void> }
...
export function createA2aRoomPolicy(getConfig: () => A2aConfig = readA2aConfig): SlackBotInboundPolicy {
  ...
  return {
    async decideBotInbound(ctx) {
      ...
      const enforcement = messageEnforcer ? await messageEnforcer(ctx) : { action: 'allow' as const };
      if (enforcement.action === 'drop') return enforcement;
      return {
        action: 'admit',
