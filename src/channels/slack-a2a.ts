...
import { setBotInboundPolicy, type SlackBotInboundContext, type SlackBotInboundPolicy } from './slack-a2a-guard.js';
...
/** Optional host-owned checkpoint layered onto allowlisted A2A rooms. */
export type A2aMessageEnforcementDecision =
  | {
      action: 'allow';
      beforeForward?: () => void | Promise<void>;
      onAccepted?: () => void | Promise<void>;
      onRejected?: () => void | Promise<void>;
      requiredAgentGroupId?: string;
    }
  | { action: 'drop'; reason: string };

export type A2aMessageEnforcer = (
  ctx: SlackBotInboundContext,
) => A2aMessageEnforcementDecision | Promise<A2aMessageEnforcementDecision>;

let messageEnforcer: A2aMessageEnforcer | null = null;

/**
 * Install one optional A2A message enforcer. The handoff-ledger module uses
 * this seam so the Slack channel stays usable without that optional module.
 */
export function setA2aMessageEnforcer(enforcer: A2aMessageEnforcer | null): void {
  if (enforcer && messageEnforcer) log.warn('slack-a2a: message enforcer overwritten');
  messageEnforcer = enforcer;
}
...
export function createA2aRoomPolicy(getConfig: () => A2aConfig = readA2aConfig): SlackBotInboundPolicy {
  /** Accepted and admitted-but-unsettled hops, keyed `<instanceKey>:<room>`. */
  const hops = new Map<string, { accepted: number; reserved: number }>();

  function release(key: string, accepted: boolean): void {
    const state = hops.get(key);
    if (!state) return;
    state.reserved = Math.max(0, state.reserved - 1);
    if (accepted) state.accepted += 1;
    if (state.accepted === 0 && state.reserved === 0) hops.delete(key);
  }
  ...
  return {
    async decideBotInbound(ctx) {
      ...
      const state = hops.get(key) ?? { accepted: 0, reserved: 0 };
      if (state.accepted + state.reserved >= maxHops) {
      ...
      // Reserve synchronously before awaiting enforcement so concurrent
      // decisions observe the same budget. Failed admissions/deliveries
      // release the reservation through the paths below.
      state.reserved += 1;
      hops.set(key, state);
      let enforcement: A2aMessageEnforcementDecision;
      try {
        enforcement = messageEnforcer ? await messageEnforcer(ctx) : { action: 'allow' as const };
      } catch (err) {
        release(key, false);
        throw err;
      }
      ...
      return {
        ...
        requiredAgentGroupId: enforcement.requiredAgentGroupId,
        ...
        beforeForward: enforcement.beforeForward,
        onAccepted: async () => {
          try {
            await enforcement.onAccepted?.();
          } finally {
            settle(true);
          }
        },
    ...
    onHumanInbound(ctx) {
      ...
      const key = `${ctx.instanceKey}:${roomFromPlatformId(ctx.platformId)}`;
      const state = hops.get(key);
      if (!state) return;
      state.accepted = 0;
      if (state.reserved === 0) hops.delete(key);
    },
  };
}
