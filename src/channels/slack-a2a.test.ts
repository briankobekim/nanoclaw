...
describe('slack-a2a room policy (registered via the channel barrel)', () => {
  ...
  it('reserves the hop budget across concurrent delivery lifecycles', async () => {
    const { createA2aRoomPolicy } = await import('./slack-a2a.js');
    const { setBotInboundPolicy } = await import('./slack-a2a-guard.js');
    setBotInboundPolicy(createA2aRoomPolicy(() => ({ rooms: new Set(['G0CONCURRENT']), maxHops: 1 })));
    let release!: () => void;
    ...
    const wrapped = wrapSlackBotGuard(setup, 'slack-concurrent');

    const attempts = Array.from({ length: 8 }, (_, index) =>
      Promise.resolve(wrapped.onInbound('slack:G0CONCURRENT', null, botMsg(`c${index}`))),
    );
